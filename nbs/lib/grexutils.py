...
import neo4j
from .neotools import NumpyStore, query_write, query_read_yield, query_read_return_list
...
"""
    return [(r["name"], r["key"]) for r in query_read_yield(driver, q)]
...
"""
    records = query_read_return_list(driver, q, name=name)
    if len(records) < 1:
...
"""
    return [(r["name"], r["key"]) for r in query_read_yield(driver, q, key=key)]


def parameter_names_keys_from_experiment_and_procedure_names(
    driver, ex_name, proc_name
):
    q = """
...
"""
    return [
        (r["name"], r["key"])
        for r in query_read_yield(driver, q, ex_name=ex_name, proc_name=proc_name)
    ]
...
"""
    return [
        (r["name"], r["key"])
        for r in query_read_yield(driver, q, ex_key=ex_key, proc_key=proc_key)
    ]


def started_parameter_names_keys_from_experiment_and_procedure_keys(
    driver, ex_key, proc_key
):
    q = """
MATCH (e:Experiment {unikey: $ex_key})
-[:INCLUDES]->(proc:Procedure {unikey: $proc_key})
...
"""
    return [r["unikey"] for r in query_read_yield(driver, q, **kwargs)]
...
def get_code_strings_from_db(driver, experiment_name, procedure_name):
    ...
    procedures = dict(
        get_procedure_names_keys_from_experiment_key(driver, key=experiment_unikey)
    )
    procedure_unikey = procedures[procedure_name]
    # print(procedure_unikey)
    unstarted_parameters = get_unstarted_parameters_of_procedure(
        driver, procedure_unikey=procedure_unikey
    )
    parameters_unikey = unstarted_parameters[0]
    # print(parameters_unikey)
    code_strings_from_db = get_code_strings_of_experiment_procedure_parameters(
        driver,
...
class UnikeyedObjectProperty:
    def __set_name__(self, owner, attr_name):
        ...
        self.fetch_q = "MATCH (n:%s {unikey: $unikey}) RETURN n.%s as %s" % (
            owner.__name__,
            attr_name,
            attr_name,
        )
    ...
    def __get__(self, obj, objtype=None):
        return query_read_return_list(obj.driver, self.fetch_q, unikey=obj.unikey)[0][
            self.attr_name
        ]
...
class NNetProperty:
    ...
    def __get__(self, obj, objtype=None):
        if not hasattr(obj, self.cache_name):
            ...
            # records = query_read_return_list(obj.driver, self.fetch_q, unikey=obj.unikey)
            # if len(records) < 1:
...
RETURN [n IN nodes(p) | n.unikey] as net_unikeys"""
    ...
    def __get__(self, obj, objtype=None):
        if not hasattr(obj, self.cache_name):
            # print(f"creating {self.name}")
            records = query_read_return_list(
                obj.driver, self.fetch_q, unikey=obj.unikey
            )
            if len(records) < 1:
            ...
            if len(records) > 1:
                raise KeyError(
                    f"Found {len(records)} from Parameter with unikey={obj.unikey}"
                )
            r = records[0]
        ...
        if not hasattr(obj, self.cache_name):
            ...
            val = Thing(
                **dict(
                    (
                        (k, list(v))
                        for k, v in groupby_transform(
                            sorted(
                                flatten(list(a.items()) for a in r), key=itemgetter(0)
                            ),
                            itemgetter(0),
                            itemgetter(1),
                            collapse,
