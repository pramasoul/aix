...
class NNVis:
    ...
    def knobs_plot_learning(self, batches=100, batch_size=1):
        ...
        def sfunc(x):
            return 2 ** (-1.005 / (x + 0.005))

        def sinv(x):
            return (-1.005 / math.log2(x)) - 0.005
        ...
        wph = 0.6
        weights_axes = [
            plt.axes([0.025, wpy - wph * (i + 1) / nl, 0.10, (wph - 0.1) / nl])
            for i in range(nl)
        ]

        def make_iax_images():
            return [
                weights_axes[i].imshow(
                    np.concatenate(
                        (
    ...
    def plot_trajectory(self, traja):
        ...
        ax.set_yscale("log")
        (tnl,) = ax.plot(
            traja.traj_L2, label=f"traj norm", color=traj_color
        )  # Plot some data on the axes.
        ax2 = ax.twinx()  # instantiate a second axes that shares the same x-axis
        ax2.tick_params(axis="y", labelcolor=loss_color)
        (dll,) = ax2.plot(
            traja.loss_steps, label=f"$\Delta loss$", color=loss_color
        )  # Plot some data on the axes.
        # dll, = ax2.plot(np.tanh(10*traja.loss_steps), label=f"$\tanh(\Delta loss)$", color=loss_color)  # Plot some data on the axes.
        (cosl,) = ax2.plot(
            traja.traj_cos, label=f"$\Delta state cosine$", color=cos_color
        )
        ax.legend(
            [tnl, dll, cosl],
            ["$\\|\\Delta state \\|$", "$\\Delta loss$", "$cos(\\theta)\Delta$"],
        )  # Add a legend.
...
class ADCResponsePlot:
    def __init__(
        ...
        **kwargs,
        ...
        x = np.arange(-0.125, 1.125, resolution)
        xs = bq.LinearScale()
        ys = bq.LinearScale()
        ...
        fig.fig_margin = kwargs.get("fig_margin") or dict(
...
if False:  # boneyard
    ...
    def mpl_plot_loss_cube(self, cube=None):
        ...
        surf = ax.plot_surface(
            XX, YY, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False
        )
