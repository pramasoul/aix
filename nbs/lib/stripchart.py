    ...
    def __call__(self, new_data):
        ...
        if l_new_data >= self.data_len:
            ...
            self.data = np.concatenate(
                (self.data[-l_old_to_keep:], new_data[-l_new_to_add:])
            )
        data = self.data
...
class BQStripchart:
    def __init__(self, data_len, **kwargs):
        ...
        scat = self.scat = bq.Scatter(
            x=x, y=data, scales={"x": xs, "y": ys}, default_size=5
        )
    ...
    def __call__(self, new_data):
        ...
        with self.fig.hold_sync():
            scat.x = np.arange(
                self.last_batch_number - data.size, self.last_batch_number
            )
            scat.y = data
...
class Losschart:
    def __init__(
        ...
        **kwargs
        ...
        self.fig = bq.Figure(marks=[scat], axes=[xax, yax], title="Loss")
        self.fig.fig_margin = kwargs.get("fig_margin") or dict(
